...
# Datos Iniciales
name = input("De quién vamos a actualizar los datos?: ")
mes = input("En qué mes fue el torneo? ")

if os.path.isfile("./pj/" + name + ".txt"):
    j0 = open("./pj/" + name + ".txt", "r")
    lines = j0.readlines()
    j0.close()
    j0 = lines[len(lines) - 1]
    xi = j0.split()
    J0 = []
    for z in range(0, 5):
        q = float(xi[z])
...
# Datos de partidas
P = int(input("Cuántas partidas jugó? "))

for i in range(0, P):
    ji = input("Contra quién jugó? ")
    if os.path.isfile("./pj/" + ji + ".txt"):
        ji0 = open("./pj/" + ji + ".txt", "r")
        lines = ji0.readlines()
        ji0.close()
        ji0 = lines[len(lines) - 1]
        xi = ji0.split()
        Ji = []
        for z in range(0, 5):
            q = float(xi[z])
...
for i in range(P):
    si = int(input("Resultados " + str(i + 1) + ": "))
    S.append(si)
...
# Guardados
with open("rank/" + mes + ".txt", "a+") as rank:
    rank.write(name + ": ")
    rank.write(ND)
    rank.write("\n")
    rank.write("\n")